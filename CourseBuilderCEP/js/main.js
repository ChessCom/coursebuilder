/* ── CEP bridge check ──────────────────────────────────────────────────────
   window.__adobe_cep__ is injected by the Premiere Pro CEP runtime.
   If absent, the extension is not running inside PP.
────────────────────────────────────────────────────────────────────────── */
...
/* ── Safe eval (no dependency on host.jsx functions) ───────────────────── */
...
/* ── GitHub URL for course_builder.jsx ──────────────────────────────────── */
var JSX_URL = 'https://raw.githubusercontent.com/ChessCom/coursebuilder/main/course_builder.jsx';

/* ── Derive sibling tool paths from the extension directory ─────────────── */
...
/* Auto-detect extension directory to derive sibling tool paths */
(function() {
    try {
        var _fs   = require('fs');
...
/* ── Download JSX from URL (follows redirects) ───────────────────────────── */
...
/* Fetch on panel load to show bullet status early */
setScriptStatus('loading script...', 'loading');
downloadRemoteJsx(JSX_URL,
    function(p) { setScriptStatus('script loaded', 'ok'); },
...
function refreshLog() {
    ...
    safeEvalScript(logScript, function (content) {
...
/* ── Detect course from test clip in "test2" sequence ─────────────────── */
function detectCourse() {
    ...
    safeEvalScript(detectScript, function (result) {
        if (!result) {
...
try {
    var _sp = localStorage.getItem('ac_py_path');
    if (_sp) acPathInput.value = _sp;
    var _sc = localStorage.getItem('ac_chapter');
    if (_sc) acChapterInput.value = _sc;
} catch(e) {}
...
function runAutocut(singleChapter) {
    ...
    if (!pyPath) {
        acStatus.textContent = 'ERROR: enter the path to autocut.py';
        acStatus.className   = 'status error';
        return;
    }
    if (singleChapter && !chapter) {
        acStatus.textContent = 'ERROR: enter the chapter name';
        acStatus.className   = 'status error';
        return;
    }
    ...
    acStatus.textContent  = singleChapter
        ? 'AutoCut: testing "' + chapter + '"…'
        : 'AutoCut: processing full course…';
    ...
    proc.stderr.on('data', function (chunk) {
        appendLine('ERR: ' + chunk.toString());
    });
    proc.on('close', function (code) {
        if (buf) appendLine(buf + '\n');
        btnAcTest.disabled   = false;
        btnAcRun.disabled    = false;
        btnAcTest.textContent = '▶ Test Chapter';
        btnAcRun.textContent  = '▶ Run Course';
        if (code === 0) {
            acStatus.textContent = singleChapter ? 'Test completed.' : 'Course completed.';
            acStatus.className   = 'status done';
        } else {
            acStatus.textContent = 'AutoCut finished with code ' + code;
            acStatus.className   = 'status error';
        }
    });
...
try {
    var _bcp = localStorage.getItem('bc_py_path');
    if (_bcp) bcPathInput.value = _bcp;
} catch(e) {}
...
function _bcApplyJsx(ti, ci, mp, left, top, right, bottom, clipW, clipH, mode) {
    ...
    var DX=0,DY=2;  // position offset for chessbase: right, down (px)
    var IL=0.15,IR=0.20,IT=0.00,IB=0.95,SB=1.010;
    // chess.com: scale based on full clip (no zoom-in), crop removes sidebar
    var isCC = (mode === 'chesscom');
    return '(function(){' +
        'var seq=app.project.activeSequence;' +
        'if(!seq)return "error:no seq";' +
        'var tr=seq.videoTracks['+ti+'];' +
        'if(!tr)return "error:no track";' +
        'var cl=tr.clips['+ci+'];' +
        'if(!cl)return "error:no clip";' +
        ...
        // dump component names for diagnostics (only first call)
        ...
        'for(var i=0;i<cl.components.numItems;i++){' +
            'if(cl.components[i].displayName==="Motion"){mo=cl.components[i];break;}' +
        '}' +
        ...
        'for(var ri=0;ri<mo.properties.numItems;ri++){' +
            ...
            'try{' +
                'var rkc=rp.keyframeCount;' +
                'for(var rki=rkc-1;rki>=0;rki--)try{rp.removeKey(rki);}catch(e){}' +
                'if(rn==="Scale")rp.setValue(100,true);' +
                'else if(rn==="Uniform Scale")rp.setValue(1,true);' +
                'else if(rn==="Position")rp.setValue([0.5,0.5],false);' +
                'else if(rn==="Rotation")rp.setValue(0,true);' +
            '}catch(e){}' +
        '}' +
        // reset only crop-named properties (never zero Opacity or other effects)
        ...
        (isCC ?
            // chess.com: board fills the sequence, camera/sidebar go off-screen
            ...
            // chessbase: left>25% = wide clip (camera+board) → same fill-sequence approach
            // otherwise: standard clip → ChessBase BOX approach
            'var isWide=(left>25);' +
            'var s,px,py;' +
            'if(isWide){' +
        ...
        'var sc=s*100;' +
        'var nx=px/seqW,ny=py/seqH;' +
        (isCC ? 'var cL=left,cR=right,cT=top,cB=bottom;' :
                // wide chessbase: no insets (same as chess.com); standard: use insets
        ...
        // apply Motion (Scale, Position)
        'for(var pi=0;pi<mo.properties.numItems;pi++){' +
            'var p=mo.properties[pi],n=p.displayName;' +
            'try{' +
                'if(n==="Scale"){p.setValue(sc,true);log.push("scale="+Math.round(sc*10)/10+"%");}' +
                ...
                'else if(n==="Rotation")p.setValue(0,true);' +
            '}catch(e){log.push(n+":ERR="+e.message);}' +
        '}' +
        // apply Crop: find "Crop" component; if absent try QE DOM to add it
        'var cropComp=null;' +
        ...
        'if(cropComp){' +
            'for(var pi2=0;pi2<cropComp.properties.numItems;pi2++){' +
                ...
                'try{' +
                    'if(n2==="Left"||n2==="Crop Left")p2.setValue(cL,true);' +
                    'else if(n2==="Top"||n2==="Crop Top")p2.setValue(cT,true);' +
                    'else if(n2==="Right"||n2==="Crop Right")p2.setValue(cR,true);' +
                    'else if(n2==="Bottom"||n2==="Crop Bottom"){p2.setValue(cB,true);log.push("crop:"+cL.toFixed(0)+","+cT.toFixed(0)+","+cR.toFixed(0)+","+cB.toFixed(0));}' +
                '}catch(e){}' +
            '}' +
...
function runBoardCrop(mode) {
    ...
    if (!pyPath) {
        bcStatus.textContent = 'ERROR: enter the path to crop_board_sel.py';
        bcStatus.className   = 'status error';
        return;
    }
    var cp;
    try { cp = require('child_process'); } catch(e) {
        bcStatus.textContent = 'ERROR: Node.js unavailable';
        bcStatus.className   = 'status error';
        return;
    }
    ...
    function done(msg, isErr) {
        ...
        btnEl.textContent    = '▶ BoardCrop ' + label;
        bcStatus.textContent = msg;
        bcStatus.className   = isErr ? 'status error' : 'status done';
    }

    safeEvalScript(_BC_GET_CLIPS_JSX, function(raw) {
        var data;
        try { data = JSON.parse(raw); } catch(e) {
            done('ERROR: unexpected response from PP', true); return;
        }
        if (data.error) { done('ERROR: ' + data.error, true); return; }
        var clips = data.clips;
        if (!clips || !clips.length) {
            done('ERROR: no clips selected in timeline', true); return;
        }
            ...
            for (var li = 0; li < lines.length; li++) {
                ...
                if (res && typeof res.ok !== 'undefined') {
                    if (res.ok && clipMap[res.path]) {
                        ...
                        var jsx = _bcApplyJsx(c.ti, c.ci, res.path,
                                              res.left, res.top, res.right, res.bottom,
                                              res.clipW, res.clipH, mode);
                        (function(clipName) {
                            safeEvalScript(jsx, function(applyRes) {
                                log('  apply [' + clipName + ']: ' + applyRes);
                            });
                        })(c.name || res.path);
                    } else {
                        log('  detect FAIL: ' + (res.error || 'unknown'));
                    }
                } else {
                    log(line);
                }
...
try {
    var _atp = localStorage.getItem('at_py_path');
    if (_atp) atPathInput.value = _atp;
} catch(e) {}
atPathInput.addEventListener('change', function () {
    try { localStorage.setItem('at_py_path', atPathInput.value.trim()); } catch(e) {}
});

var _AT_SKIP_NAMES = ['fade_in','fade_out','fade in','fade out',
                      'intro-outro','intro_outro','_no_audio',
                      'bg_chapters','background'];

var _AT_GET_CLIPS_JSX =
    '(function(){' +
        'var seq=app.project.activeSequence;' +
        'if(!seq)return JSON.stringify({error:"no active sequence",clips:[]});' +
        'var skip=["fade_in","fade_out","fade in","fade out",' +
                  '"intro-outro","intro_outro","_no_audio",' +
                  '"bg_chapters","background"];' +
        'function shouldSkip(n){' +
            'var nl=n.toLowerCase();' +
            'for(var si=0;si<skip.length;si++)if(nl.indexOf(skip[si])>=0)return true;' +
            'return false;' +
        '}' +
        ...
        'for(var vi=0;vi<seq.videoTracks.numTracks;vi++){' +
            ...
            'for(var ci=0;ci<tr.clips.numItems;ci++){' +
                ...
                'if(!isVid)continue;' +
                'var cname=cl.name||"";' +
                'if(shouldSkip(cname)||shouldSkip(mp))continue;' +
...
function runAutoTrim() {
    ...
    if (!pyPath) {
        atStatus.textContent = 'ERROR: enter the path to claptrim.py';
        atStatus.className   = 'status error';
        return;
    }
    var cp;
    try { cp = require('child_process'); } catch(e) {
        atStatus.textContent = 'ERROR: Node.js unavailable';
        atStatus.className   = 'status error';
        return;
    }
    ...
    log('[AutoTrim JS v6b | panel v108]');
    function done(msg, isErr) {
        btnAt.disabled       = false;
        btnAt.textContent    = '▶ AutoTrim (clap + speech)';
        atStatus.textContent = msg;
        atStatus.className   = isErr ? 'status error' : 'status done';
    }
    ...
    safeEvalScript(_AT_GET_CLIPS_JSX, function(raw) {
        ...
        var clips = data.clips;
        if (!clips || !clips.length) {
            done('ERROR: no video clips in active sequence', true); return;
        }
            ...
            for (var li = 0; li < lines.length; li++) {
                ...
                if (res && typeof res.ok !== 'undefined') {
                    if (res.ok && clipMap[res.path]) {
                        ...
                        log('  FAIL [' + (res.path || '?') + ']: ' + (res.error || 'unknown'));
                    }
                } else {
                    log(line);
                }
...
var MS_FILL_PY   = '/Users/raulmartinez/Desktop/chess.com/AI/ChessTools/ManualSync/manual_sync_full.py';
...
function msNavShow(show) {
    document.getElementById('msChapterNav').style.display = show ? 'block' : 'none';
}

function msUpdateNav() {
    var total = _msChapters.length;
    var cur   = _msCurrentCh;
...
function loadChessableJSON(courseId, cb) {
    var fs;
    try { fs = require('fs'); } catch(e) { cb('Node unavailable'); return; }
    var p = COURSE_JSON(courseId);
    if (!fs.existsSync(p)) {
        cb('Not found: ' + p + '\nRun: python3 /tmp/get_chessable_data.py');
        return;
    }
    try {
        var data = JSON.parse(fs.readFileSync(p, 'utf8'));
        cb(null, data.chapters || []);
...
function matchAndCompute(chessChapters, pmChapters) {
    var pmMap = {};
    pmChapters.forEach(function(pm) {
        pmMap[normName(pm.chapter_name)] = pm;
    });
    ...
    chessChapters.forEach(function(ch) {
        ...
        if (!pm) {
            ...
            if (chessNum) {
                Object.keys(pmMap).forEach(function(k) {
                    if (!pm && k.indexOf(chessNum) === 0) pm = pmMap[k];
                });
            }
        }

        var fps   = pm ? (pm.fps || 25) : 25;
        var frame = 1 / fps;
        var clips = pm ? pm.clips : [];
        ...
        out.push({
            ...
            chapter_index: ch.chapter_index,
            premiere_name: pm ? pm.chapter_name : null,
            variations:    vars,
        });
    });
    return out;
...
function buildChapterHTML(ch, idx) {
    ...
    for (var vi = 0; vi < ch.variations.length; vi++) {
        ...
        rows += '<div class="row '+(hasTc?'ok':'skip')+'">' +
            ...
            '<div class="tc">'+(hasTc
                ? '<b>'+v.timecode_start+'</b><br><small style="color:#aaa">→ '+(v.timecode_end||'?')+'</small>'
    ...
    return '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">' +
        ...
        '.vlink{font-size:.66rem;color:#4fc3f7;text-decoration:none}' +
        '.board img{max-width:140px;border-radius:3px}' +
        '</style></head><body>' +
        '<h1>&#9822; '+ch.chapter_name+'</h1>' +
...
document.getElementById('btnMs').addEventListener('click', function() {
    ...
    safeEvalScript(READ_TIMECODES_JSX, function(raw) {
        ...
        loadChessableJSON(courseId, function(err, chessChapters) {
            ...
            var fs;
            try { fs = require('fs'); } catch(e) { msStatus('Node unavailable', true); return; }

            var chapters = matchAndCompute(chessChapters, pmChapters);

            fs.writeFileSync('/tmp/ms_timestamps.json',
                JSON.stringify({ course_id: courseId, chapters: chapters,
                    premiere_chapters: pmChapters }, null, 2), 'utf8');
            ...
            if (_msChapters.length === 0) {
                msStatus('⚠ No Premiere chapters match Chessable. See log.', true);
                return;
            }
...
document.getElementById('btnMsVerify').addEventListener('click', function() {
    ...
    fillStatus.textContent = 'Extracting frames and comparing boards…';
    fillStatus.className   = 'status';
    var SCRIPT = '/Users/raulmartinez/Desktop/chess.com/AI/ChessTools/ManualSync/board_sync.py';
    var cmd = HOME + '/msenv/bin/python3 "' + SCRIPT + '"'
        + ' --course 424926'
        + ' --chapter-index ' + ch.chapter_index
        + ' 2>&1';
    cp.exec(cmd, {timeout: 300000}, function(err, stdout, stderr) {
...
document.getElementById('btnMsFill').addEventListener('click', function() {
    ...
    var ch = _msChapters[_msCurrentCh];
    if (!ch) return;

    var fillStatus = document.getElementById('msFillStatus');
    fillStatus.textContent = 'Filling "' + ch.chapter_name + '"…';
    fillStatus.className   = 'status';

    var chIdx    = ch.chapter_index;
    var courseId = (document.getElementById('msCourseId').value || '424926').trim();
...
document.getElementById('btnAt5sec').addEventListener('click', function() {
    ...
    var jsx =
        '(function(){' +
            ...
            'for(var vi=0;vi<seq.videoTracks.numTracks;vi++){' +
                ...
                'for(var ci=0;ci<tr.clips.numItems;ci++){' +
                    ...
                    'if(!mp)continue;' +
                    'var bIn=cl.inPoint.seconds.toFixed(2);' +
                    'var origDur=cl.end.seconds-cl.start.seconds;' +
        ...
        '})()';
    safeEvalScript(jsx, function(res) {
...
document.getElementById('btnCreatePreviews').addEventListener('click', function() {
    var logEl    = document.getElementById('log');
    var psStatus = document.getElementById('psStatus');
    var btn      = document.getElementById('btnCreatePreviews');
    btn.disabled = true;
    psStatus.textContent = 'Creating preview sequences…';
    psStatus.className   = 'status';
    ...
    var jsx =
        '(function(){' +
            ...
            // Walk the project tree; when we find an item matching nodeId, return its parent bin
            'function findParentBin(bin,nodeId){' +
                'if(!bin.children)return null;' +
                'for(var i=0;i<bin.children.numItems;i++){' +
                    'var c=bin.children[i];' +
                    'try{if(c.nodeId===nodeId)return bin;}catch(e){}' +
                    'if(c.type===2){var r=findParentBin(c,nodeId);if(r)return r;}' +
                '}return null;' +
            '}' +

            // Snapshot the sequence list BEFORE any cloning so we never clone a clone
            'var snap=[];' +
            'for(var si=0;si<app.project.sequences.numSequences;si++){' +
                'var s=app.project.sequences[si];' +
                'var nid=null;try{nid=s.projectItem.nodeId;}catch(e){}' +
                'snap.push({seq:s,nm:s.name,nodeId:nid});' +
            '}' +

            // Build set of all existing names (to skip pre-existing previews)
            'var existingNames={};' +
            'for(var ei=0;ei<snap.length;ei++) existingNames[snap[ei].nm]=1;' +

            'for(var si=0;si<snap.length;si++){' +
                'var seq=snap[si].seq, nm=snap[si].nm, nodeId=snap[si].nodeId;' +
                'var nmLow=nm.toLowerCase();' +
                'var previewNm=nm+SUFFIX;' +

                // Skip sequences that already end with _preview
                'if(nm.length>=SUFFIX.length&&nm.substr(nm.length-SUFFIX.length)===SUFFIX)' +
                    '{continue;}' +

                // Skip if preview already exists
                'if(existingNames[previewNm]){skipped.push(nm+" (already exists)");continue;}' +

                // Skip template / webcam / nested sequences
                'if(nmLow.indexOf("webcam_")===0||nmLow.indexOf("nested")>=0||' +
                    'nm==="void"||nm==="test2"||nm==="test")' +
                    '{skipped.push(nm);continue;}' +

                // Find parent bin by traversing the tree with the nodeId
                'var parentBin=nodeId?findParentBin(app.project.rootItem,nodeId):null;' +

                // Skip sequences in the "Peek Inside" bin
                'if(parentBin&&parentBin.name.toLowerCase().indexOf("peek inside")>=0)' +
                    '{skipped.push(nm);continue;}' +
                ...
                'try{' +
                    ...
                    'for(var bi=0;bi<app.project.sequences.numSequences;bi++)' +
                        'before[app.project.sequences[bi].name]=1;' +

                    'var cloned=null;' +
                    'try{cloned=seq.clone();}' +
                    'catch(ce){errors.push(nm+": clone: "+ce.message);continue;}' +
                    '$.sleep(500);' +

                    // Find the newly created sequence
                    'var newSeq=null;' +
                    'if(cloned&&typeof cloned.name!=="undefined")newSeq=cloned;' +
                    'if(!newSeq){' +
                        'for(var fi=0;fi<app.project.sequences.numSequences;fi++){' +
                            'var fs=app.project.sequences[fi];' +
                            'if(!before[fs.name]){newSeq=fs;break;}' +
                        '}' +
                    '}' +
                    ...
                    // Move to same bin as original
                    'if(parentBin){' +
                        'try{newSeq.projectItem.moveBin(parentBin);}' +
                        'catch(e){errors.push(previewNm+": moveBin: "+e.message);}' +
                    '}else{' +
                        'errors.push(previewNm+": parentBin not found (nodeId="+nodeId+")");' +
                    '}' +
...
document.getElementById('btnCutPreview').addEventListener('click', function() {
    ...
    var jsx =
        '(function(){' +
            ...
            'if(!pos)return "error: cannot get player position";' +
            ...
            // Find fade_out project item
            'function findFadeOut(item){' +
                'if(!item)return null;' +
                'if(item.name&&item.name.toLowerCase().indexOf("fade_out")>=0&&item.type!==2)return item;' +
                'try{if(item.children)for(var i=0;i<item.children.numItems;i++){' +
                    'var r=findFadeOut(item.children[i]);if(r)return r;' +
                '}}catch(e){}' +
                'return null;' +
            '}' +
            'var fadeOutItem=findFadeOut(app.project.rootItem);' +

            // Trim/delete video clips
            'for(var vi=0;vi<seq.videoTracks.numTracks;vi++){' +
                'var vtr=seq.videoTracks[vi],vDel=[];' +
                'for(var vci=0;vci<vtr.clips.numItems;vci++){' +
                    'var vcl=vtr.clips[vci];' +
                    'if(vcl.start.seconds>=cutSec)vDel.push(vci);' +
                    'else if(vcl.end.seconds>cutSec){' +
                        'try{var vt=new Time();vt.seconds=cutSec;vcl.end=vt;}catch(e){log.push("trimV"+(vi+1)+":"+e.message);}' +
                    '}' +
                '}' +
                'for(var vri=vDel.length-1;vri>=0;vri--)' +
                    'try{vtr.clips[vDel[vri]].remove(false,false);}catch(e){log.push("delV"+(vi+1)+":"+e.message);}' +
            '}' +

            // Trim/delete audio clips
            'for(var ai=0;ai<seq.audioTracks.numTracks;ai++){' +
                'var atr=seq.audioTracks[ai],aDel=[];' +
                'for(var aci=0;aci<atr.clips.numItems;aci++){' +
                    'var acl=atr.clips[aci];' +
                    'if(acl.start.seconds>=cutSec)aDel.push(aci);' +
                    'else if(acl.end.seconds>cutSec){' +
                        'try{var at=new Time();at.seconds=cutSec;acl.end=at;}catch(e){}' +
                    '}' +
                '}' +
                'for(var ari=aDel.length-1;ari>=0;ari--)' +
                    'try{atr.clips[aDel[ari]].remove(false,false);}catch(e){}' +
            '}' +

            // Place fade_out on top video track (V4 preferred; add it if missing)
            'if(fadeOutItem){' +
                ...
                'if(!topTrack){' +
                    'try{' +
                        ...
                        'if(qs&&typeof qs.addTracks==="function"){qs.addTracks(1,0);$.sleep(400);}' +
                    '}catch(e){}' +
                    'topTrack=seq.videoTracks.numTracks>=4?seq.videoTracks[3]:seq.videoTracks[seq.videoTracks.numTracks-1];' +
                    'log.push("V="+seq.videoTracks.numTracks);' +
                '}' +
                'var fadeStart=Math.max(0,cutSec-0.7);' +
                'var ft=new Time();ft.seconds=fadeStart;' +
                'try{' +
                    'topTrack.overwriteClip(fadeOutItem,ft);' +
                    '$.sleep(200);' +
                    'for(var fci=topTrack.clips.numItems-1;fci>=0;fci--){' +
                        'var fcl=topTrack.clips[fci];' +
                        'if(Math.abs(fcl.start.seconds-fadeStart)<0.15){' +
                            'try{var fet=new Time();fet.seconds=cutSec;fcl.end=fet;}catch(e){}' +
                            'log.push("fade@"+fadeStart.toFixed(2)+"→"+cutSec.toFixed(2)+"s ✓");' +
                            'break;' +
                        '}' +
                    '}' +
...
document.getElementById('btnColorCode').addEventListener('click', function() {
    ...
    btn.disabled = true;
    ccStatus.textContent = 'Coloring…';
    ccStatus.className   = 'status';

    // PP label integers: 5=Cerulean, 16=Yellow, 8=Mango, 3=Caribbean,
    //   7=Rose, 6=Forest, 12=Magenta, 4=Lavender, 14=Green, 11=Teal
    var jsx =
        '(function(){' +
            'var SEQ_COL=16,CAM_COL=5;' +
            'var BIN_COLS=[8,3,7,6,12,4,14,11];' +
            'var seqCount=0,camCount=0,binCount=0;' +
            // color all non-bin project items recursively
            'function colorSeqs(item){' +
                'if(!item.children)return;' +
                'for(var i=0;i<item.children.numItems;i++){' +
                    'var c=item.children[i];' +
                    'if(c.type===2){colorSeqs(c);continue;}' +
                    'var nm=c.name;' +
            ...
            // find a bin by name (depth-first)
            'function findBin(parent,name){' +
                'if(!parent.children)return null;' +
                'for(var i=0;i<parent.children.numItems;i++){' +
                    'var c=parent.children[i];' +
                    'if(c.type===2&&c.name===name)return c;' +
                    'if(c.type===2){var r=findBin(c,name);if(r)return r;}' +
                '}return null;' +
            '}' +
            // color immediate child bins of 03_Course
            'var course=findBin(app.project.rootItem,"03_Course");' +
            'if(course){' +
                'for(var i=0;i<course.children.numItems;i++){' +
                    'var child=course.children[i];' +
                    'if(child.type===2){' +
                        'try{child.label=BIN_COLS[binCount%BIN_COLS.length];binCount++;}catch(e){}' +
                    '}' +
                '}' +
            '}' +
